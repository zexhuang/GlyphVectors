...
class EarlyStopping:
    ...
    if validation monitor doesn't improve after a given patience.
    """
    def __init__(self, path,
        ...
        Args:
            ...
            Default: None
            patience (int): How long to wait
            after last time validation monitor improved.
            Default: 10
            delta (float): Minimum change in the monitored quantity
            to qualify as an improvement.
            Default: 0
            verbose (bool): If True, prints a message
            for each validation monitor improvement.
            Default: False
    ...
    def __call__(self, monitor, cm, model, optimizer, epoch):
        ...
        if self.best_score is None:
            ...
            self.counter += 1
            self.trace_func(f"Validation monitor does not imporove ({self.best_score}\
                              --> {score}).\
    ...
    def save_checkpoint(self, checkpoint, score):
        ...
        if self.verbose:
            ...
            self.trace_func(f'Validation monitor decrease/increase ({self.best_score:.6f}\
                              --> {score:.6f}).\
                              \n Saving model ...')
